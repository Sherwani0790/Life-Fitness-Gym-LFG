...
import { LayoutDashboard, Users, LogOut, ChevronLeft, ChevronRight, Briefcase, Dumbbell, Box } from 'lucide-react';
import { Link, useLocation, useNavigate } from 'react-router-dom';
import { Tooltip, TooltipContent, TooltipProvider, TooltipTrigger } from '@/src/UI-Components/tooltip';
...
export function Sidebar({ isCollapsed, setIsCollapsed, isMobileMenuOpen, setIsMobileMenuOpen }: SidebarProps) {
  ...
  const hoverClass = "hover:bg-sidebar-foreground/10 hover:text-sidebar-foreground";
    ...
    <TooltipProvider delayDuration={0}>
      ...
      >
        ...
        <div className={`h-16 flex items-center ${isCollapsed ? 'justify-center' : 'px-6'} border-b border-sidebar-border transition-all duration-300`}>
          <Logo className={isCollapsed ? "h-8 w-8" : "h-8 w-8"} showText={!isCollapsed} />
        </div>

        {/* Navigation */}
        <div className="flex-1 flex flex-col py-4 overflow-y-auto overflow-x-hidden">
          <div className={`px-3 mb-2 transition-opacity duration-300 ${isCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>
              <h2 className={`mb-2 px-4 text-xs font-semibold tracking-tight uppercase opacity-70 ${textClass}`}>
                  Platform
              </h2>
          </div>
          ...
          <div className={`flex items-center mb-4 ${isCollapsed ? 'justify-center' : ''}`}>
              ...
              {!isCollapsed && (
                  <div className="ml-3 overflow-hidden">
                      ...
                      <p className={`text-xs font-medium truncate opacity-70 ${textClass}`}>
                          {Array.isArray(userRole) ? userRole.join(', ') : 'System Admin'}
                      </p>
                  </div>
              )}
          </div>
