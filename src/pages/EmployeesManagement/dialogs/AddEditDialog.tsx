...
export function AddEditDialog({ isOpen, onClose, onSubmit, isEditing, selectedEmployee }: AddEditDialogProps) {
  const {
    register,
    handleSubmit,
    reset,
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="sm:max-w-[425px] p-0 overflow-hidden border-none shadow-2xl">
        <DialogHeader className="p-6 bg-transparent text-foreground border-b border-border/50">
          <DialogTitle className="text-lg font-bold">{isEditing ? 'Edit Employee' : 'Add New Employee'}</DialogTitle>
          <DialogDescription className="text-muted-foreground text-xs">
            {isEditing ? 'Update the employee details below.' : 'Enter the employee details to create a new account.'}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4 bg-background">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="role" className={cn("text-xs font-semibold", errors.role && "text-destructive")}>
            ...
            >
              <SelectTrigger className={cn("h-9 text-sm", errors.role && "border-destructive focus-visible:ring-destructive")}>
            ...
            {errors.role && <p className="text-[10px] font-medium text-destructive">{errors.role.message}</p>}
          </div>
          ...
          <DialogFooter className="pt-4 gap-2">
            ...
            <Button type="submit" className="h-9 text-xs px-6">{isEditing ? 'Update Employee' : 'Save Employee'}</Button>
          </DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
